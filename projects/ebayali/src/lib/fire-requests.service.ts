...
import { CommonService, ident, itemforTransfer } from './common.service';
...
export class FireRequestsService {     
  ...
  getSettings() {
    return this.db.list('/settings/').snapshotChanges()          
  }
  getShipPolicy(code:string) {
    return this.db.list('/policies/'+code.substring(code.length-8)).snapshotChanges()          
  }
  getOne (code:string, par:string) {
    return this.db.list('/'+par+'/'+code).snapshotChanges().pipe(take(1))
  }
  // getOneProduct (code:string) {
  //   return this.db.list('/aliItems/'+code).snapshotChanges().pipe(take(1))
  // }
  // getShippingsForProduct(code:string) {
  //   return this.db.list('/aliShips/'+code).snapshotChanges().pipe(take(1))
  // }
  //--------------- for Chrome Extension ----------------------------
  ...
  pushItemsToDb (forRecording: any[], task:string) {   
    ...
    forRecording.forEach(item =>{
      ...
      if (task=='settings') {
        multiPaths['settings/' + item.setting] = item.value;          
      }
      if (task=='listing') {
        multiPaths['policies/' + item.code.substring(item.code.length-8) ] = {time: item.time, countries: item.countries};
      }
    })            

    this.db.database.ref('/').update(multiPaths).then(
      ()=>{
        if (task=='products' || task=='shipping') {
          forRecording.map(item=>{                                
            if (!item.new) this.common.updateStatusEvent$.next( {task: task, status: {...this.common.ident(item), msg: "recorded_successfully", type:'OK'} } )
          })
          console.log(forRecording.length + " " + task + " - Recorded Successfully! ");
        } 
        if (task=='settings') console.log("Settings - Recorded Successfully! ");
        if (task=='listing') console.log("1 Listing - Recorded Successfully! ");
